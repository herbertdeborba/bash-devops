...
function showusage {
  ...
  echo "	-q (opcional): desabilita a impressão de logs na stdout."
  exit ${ERROR}
}
...
while getopts "s:d:t:mfq" option ; do
        case "${option}" in
                s)
					origem=${OPTARG}
                ;;
...
function do_swap {
	...
	cp ${origem} ${pathnmtmp}
	if [ $? -ne 0 ] ; then
		do_log "Copia do arquivo 1 para arquivo auxiliar \"${pathnmtmp}\" não pode ser realizada, procedimento abortado."
		do_log "$(get_now) Procedimento '${MYNME}' abortado."
		exit ${ERR_FILE_OR_PATH_NOT_FOUND}
	else
		do_log "Copia do arquivo 1 para arquivo auxiliar realizada com sucesso."
	fi
	cp ${destino} ${origem}
	if [ $? -ne 0 ] ; then
	...
	cp ${pathnmtmp} ${destino}
	if [ $? -ne 0 ] ; then
		do_log "Copia do arquivo auxiliar para \"${destino}\" não pode ser realizada, o procedimento falhou."
		do_log "$(get_now) Procedimento '${MYNME}' abortado."
		exit ${ERR_FILE_OR_PATH_NOT_FOUND}
	else
